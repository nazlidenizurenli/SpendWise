...
class TestBudgetRoutes:
    ...
    def test_get_suggested_budgets(self):
        """Test getting suggested budgets based on spending patterns"""
        # Create mock user
        mock_user = User(
            id=uuid4(),
            username="testuser",
        ...
        mock_suggestions = [
            BudgetCreate(
                limit=90.0,
                category="Food",
                description="Suggested budget for Food based on your spending patterns",
                start_date=datetime.now(),
                end_date=datetime.now() + timedelta(days=30),
                is_active=True
            )
        ...
        try:
            with patch("app.routes.budget.get_db", side_effect=mock_get_db), \
                 patch("app.services.budget.BudgetService.can_user_create_budget", return_value=True), \
                 patch("app.services.budget.BudgetService.get_suggested_budgets", return_value=mock_suggestions):
                
                response = client.get("/budgets/suggestions")
                
                assert response.status_code == 200
                suggestions = response.json()
                assert len(suggestions) == 1  # One category (Food)
                
                food_suggestion = suggestions[0]
                assert food_suggestion["category"] == "Food"
                assert food_suggestion["limit"] == 90.0
        finally:
            app.dependency_overrides.clear()
